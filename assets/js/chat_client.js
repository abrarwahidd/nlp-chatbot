var websocket;
var websocket_url = "ws://192.168.1.5:6789/";
  ...
  if (websocket.readyState == 1) {
    ...
    if ($(".connection_message").length == 0)
      $(".chatbot_client_container").append(
        '<div class="connection_message">Waiting for connection...</div>'
      );
    if (websocket.readyState == 3) initWebSocket();
  }
}
...
function initWebSocket() {
  ...
  websocket.onmessage = function (event) {
    ...
    if (data.type == "connection_id") {
      ...
      $(".user_message").remove();
      $(".chatbot_client_container").append(
        '<div class="bot_message"><div>' + data.message + "</div></div>"
      );
      ...
      for (var i = 0; i < data.options.length; i++) {
        options_html +=
          '<div class="food_option">' +
          "<h2>" +
          data.options[i]["name"] +
          " <span>" +
          data.options[i]["price"] +
          "$</span></h2>" +
          '<img src="assets/images/food_images/' +
          data.options[i]["id"] +
          '.jpg" />' +
          "</div>";
      }
      $(".chatbot_client_container").append(
        '<div class="bot_message"><div>' + options_html + "</div></div>"
      );
      scrollContainer();
    } else {
      $(".chatbot_client_container").append(
